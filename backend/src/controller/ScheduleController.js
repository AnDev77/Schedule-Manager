...
const getSchedules = (req, res) => {
    let {calStartDate, calEndDate, user_id} = req.query;
    user_id = parseInt(user_id);
    tempStart = new Date(calStartDate);
    if (!calEndDate){
        calEndDate = new Date(calStartDate);
        calEndDate.setDate(calEndDate.getDate() + 27)
        while (true){
            calEndDate.setDate(calEndDate.getDate() + 1);
            if(calEndDate.getMonth() - tempStart.getMonth() >= 1){
                calEndDate.setDate(calEndDate.getDate() - 1)
                break;
            }
        }
...
const addSchedule = (req, res) => { 
    const {user_id, title, start_date, end_date, repet_type} = req.body;

    let query = `INSERT INTO schedules (user_id, title, start_date, end_date, repet_type) VALUES(?,?,?,?,?)`;
    let values = [user_id, title, start_date, end_date, repet_type];
...
const updateSchedule = (req, res) =>{
    const {id} = req.params;
    const {schedule_title} = req.body;
    let values = [schedule_title, id];
    let query = `UPDATE schedules SET title = ? WHERE id = ?;`
